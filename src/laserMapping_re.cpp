...
void pointBodyToWorld(PointType const* const pi, PointType* const po)
{
    ...
    V3D p_global(state_point.rot.matrix() * (state_point.offset_R_L_I.matrix() * p_body + state_point.offset_T_L_I) +
                 state_point.pos);
...
int main(int argc, char** argv)
{
    ...
    while (ros::ok())
    {
        ...
        if (sync_packages(Measures))  //把一次的IMU和LIDAR数据打包到Measures
        {
            ...
            std::cout << "feats_down_size: " << feats_down_size << "  Whole mapping time(ms):  " << (t11 - t00) * 1000
                      << std::endl
                      << std::endl;
        }
